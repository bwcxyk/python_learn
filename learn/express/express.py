...
# -*- coding: utf-8 -*-
"""
@Time    : 2021/6/21 16:20
@Author  : YaoKun
@Usage   : python express
"""
...
for item in address:
    # print(item)
    province = item[1][0:3]  # 截取地址信息前三个字符
    special = special_address.get(province)  # get()返回special_address里的value
    # print(special)
    if special:  # 如果是特殊地址，转换特殊地址
        province = special
    if province in new_address:  # 如果地址在分拣字典，追加信息
        # print(new_address[province])
        new_address[province].append(item)
    else:  # 否则，插入
        new_address[province] = [item]
